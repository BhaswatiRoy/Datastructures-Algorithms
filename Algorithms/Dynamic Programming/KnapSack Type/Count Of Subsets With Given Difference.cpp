...
/*
Count Subsets With Difference - We have to count total subset partitions which creates 2 subsets with given difference
*/
...
int main()
{
	...
	int totalsum=0,n=nums.size(),diff=1;
	for(int i=0;i<n;i++)
	{
		totalsum+=nums[i];
	}
	//S1-S2=diff, S1+S2=totalsum -> S1=(diff+totalsum)/2
	int sum=(diff+totalsum)/2;
	//thus we need subset counts whose sum is equal to (diff+totalsum)/2
	int ans1=countsubsetsumrecursion(sum,nums,n);
	cout<<"Count Of Subsets With Given Difference By Recurion:"<<ans1<<endl;
	vector<vector<int>>dp(n+1,vector<int>(sum+1,-1));
	int ans2=countsubsetsummemoization(sum,nums,n,dp);
	cout<<"Count Of Subsets With Given Difference By Memoization:"<<ans2<<endl;
}
