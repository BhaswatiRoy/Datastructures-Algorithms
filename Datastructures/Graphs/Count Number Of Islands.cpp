...
LC problem - only 4 neighbors (top,bottom,left,right)
GFG problem - all 8 neighbors (top, bottom, left, right, all 4 diagonals)
...
//bfs traversal with diagonals considered as neighbors - GFG problem - https://www.geeksforgeeks.org/problems/find-the-number-of-islands/1
void bfstraversal(int i, int j, int n, int m, vector<vector<char>>&grid, vector<vector<int>>&visited)
    {
        queue<pair<int,int>>q;
        q.push({i,j});
        ...
        while(!q.empty())
        {
            ...
            int col=q.front().second;
            q.pop();
            //diagonal nodes allowed so used nested loops from -1 to 1
            //hence we get all 8 combinations by nested loops
